...
const UIStrings = {
   ...
   * realm such as a frame or a worker). PH1 is a name identifying the context
   ...
   * JavaScript realms such as frames or workers). PH1 is their total size.
   * @example {1.2 MB} PH1
   */
   ...
   * (roughly, a JavaScript realm such as a frame or a worker). PH1 is their
   * total size.
   * @example {1.2 MB} PH1
   */
...
export class HeapSnapshotView extends UI.View.SimpleView implements DataDisplayDelegate, UI.SearchableView.Searchable {
  ...
  #nativeContextFilters: SpecialFilter[] = [];
  ...
  async updateNativeContextFilters(heapSnapshotProxy: HeapSnapshotModel.HeapSnapshotProxy.HeapSnapshotProxy):
    ...
    const nativeContexts = sizes.nativeContexts.slice().sort((a, b) => b.attributedSize - a.attributedSize);
    for (const nativeContext of nativeContexts) {
      ...
      filters.push({
        ...
        filterName: `nativeContext_${nativeContext.ordinal}`,
      });
    }
    filters.push({
      uiName: i18nString(
          UIStrings.objectsSharedBetweenNativeContextsS,
          {PH1: i18n.ByteUtilities.bytesToString(sizes.sharedSize)}),
      filterName: 'sharedNativeContext',
    });
    filters.push({
      uiName: i18nString(
          UIStrings.objectsNotAttributedToNativeContextS,
  ...
  static readonly ALWAYS_AVAILABLE_FILTERS: readonly SpecialFilter[] = [
  ...
  updateFilterOptions(): void {
    ...
    const createOption = (filterOption: FilterSelectOption): HTMLOptionElement => {
    ...
    createOption({uiName: '\u2014'.repeat(18), profileIndex: -1, disabled: true});

    const specialFilters = [...HeapSnapshotView.ALWAYS_AVAILABLE_FILTERS, ...this.#nativeContextFilters];
