...
export abstract class HeapSnapshot {
  ...
  private computeNativeContextAttributionSizes(nativeContextOrdinals: number[]):
      HeapSnapshotModel.HeapSnapshotModel.NativeContextSizes {
    const nodeFieldCount = this.nodeFieldCount;
    const node = this.createNode(0);
    ...
    for (const ordinal of nativeContextOrdinals) {
