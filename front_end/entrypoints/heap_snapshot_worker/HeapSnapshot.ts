...
export abstract class HeapSnapshot {
  ...
  /**
   * Correlates all V8 Context objects with source scopes and the live closures
   * that can reach them. Field classifications are computed per context.
   */
  analyzeContexts(): HeapSnapshotModel.HeapSnapshotModel.ContextAnalysisResult {
    ...
    const functionKey = (scriptId: number, start: number, end: number): string => `${scriptId}:${start}:${end}`;
    const findSourceScope = (sourceScopes: Map<string, SourceScope>, start: number,
                             end: number): SourceScope|undefined => {
    ...
    const scripts = new Map<number, ScriptInfo>();
    const scopeInfoScriptIds = new Map<number, number|undefined>();
    const contextNodes: Array<{contextNodeIndex: number, contextNodeId: number, scopeInfoNodeIndex?: number}> = [];
    ...
    for (let nodeIndex = 0; nodeIndex < this.nodes.length; nodeIndex += this.nodeFieldCount) {
      node.nodeIndex = nodeIndex;
      ...
      if (node.rawType() === this.nodeClosureType) {
        ...
        if (closureContext && scriptId !== undefined && functionStart !== undefined && functionEnd !== undefined) {
          liveClosures.push({contextNodeIndex: closureContext.nodeIndex, scriptId, functionStart, functionEnd});
        }
        continue;
      }
      ...
      scopeInfoScriptIds.set(scopeInfo.nodeIndex, scriptId);
    }
    ...
    const liveFunctionsByScript = Map.groupBy(liveFunctions.values(), liveFunction => liveFunction.scriptId);

    const resolveScopeInfoScriptId = (scopeInfoNodeIndex: number): number|undefined => {
      if (scopeInfoScriptIds.has(scopeInfoNodeIndex)) {
        return scopeInfoScriptIds.get(scopeInfoNodeIndex);
      }
      ...
      while (!seen.has(currentNodeIndex)) {
        if (scopeInfoScriptIds.has(currentNodeIndex)) {
          scriptId = scopeInfoScriptIds.get(currentNodeIndex);
          break;
        }
      ...
      for (const visitedNodeIndex of visited) {
        scopeInfoScriptIds.set(visitedNodeIndex, scriptId);
      }
      return scriptId;
    };
    ...
    for (const context of contextNodes) {
      ...
      const scriptId = resolveScopeInfoScriptId(context.scopeInfoNodeIndex);
      if (scriptId === undefined) {
    ...
    const reachableContextsByFunction = new Map<string, Set<number>>();
    ...
    const isVariableUsedInContext =
        (scriptId: number, contextNodeIndex: number, variableUses: ContextVariableUse[]): boolean => {
          for (const variableUse of variableUses) {
            const key = functionKey(scriptId, variableUse.functionStart, variableUse.functionEnd);
            let reachableContexts = reachableContextsByFunction.get(key);
            if (!reachableContexts) {
              reachableContexts = new Set();
              reachableContextsByFunction.set(key, reachableContexts);
              const liveFunctions = liveFunctionsByScript.get(scriptId) ?? [];
      ...
      for (const context of scope.contexts) {
        ...
        for (let fieldIndex = 0; fieldIndex < context.fieldValueNodeIndexes.length; ++fieldIndex) {
          ...
          if (variableUses === undefined ||
              isVariableUsedInContext(scope.scriptId, context.contextNodeIndex, variableUses)) {
